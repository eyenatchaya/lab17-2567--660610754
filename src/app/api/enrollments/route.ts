...
export const DELETE = async (request:NextRequest) => {
  ...
  //filter
  DB.enrollments = DB.enrollments.filter((enroll) => (enroll.studentId !== body.studentId) || (enroll.courseNo !== body.courseNo));
  
  //splice find -> findIndex
  /*const found_enroll_index = DB.enrollments.findIndex(
    (enroll) => enroll.courseNo === courseNo && enroll.studentId === studentId
  );

  if (found_enroll_index === -1) {
    return NextResponse.json(
      {
        ok: false,
        message: "Enrollment does not exist",
  ...
  DB.enrollments.splice(found_enroll_index, 1);
  */
